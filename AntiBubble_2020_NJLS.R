...
#Fixing the window
from <- as.Date("2020-02-15")
to <- as.Date("2020-03-30")

Bitcoin_data <- subset(Bitcoin_data, Bitcoin_data$Date <= as.Date("2020-07-01"))

#Defining Bitcoin_data for window of interest
rBitcoin_data <- subset(Bitcoin_data, Bitcoin_data$Date >= from & Bitcoin_data$Date <= to)
p <- ggplot(data = rBitcoin_data,aes(x=Date,y=Close))
...
ts_t <- 2019 + as.numeric(difftime(Bitcoin_data$Date,gtd)/365)
...
Expect_NJLSPrice_long <- subset(Expect_NJLSPrice_long,Expect_NJLSPrice_long$Date<= as.Date('2020-04-01'))


#Plot the expected price
Bitcoin_data_plot <- subset(Bitcoin_data,Bitcoin_data$Date >=as.Date('2020-02-01'))
p  <- ggplot() + geom_line(aes(Bitcoin_data_plot$Date,Bitcoin_data_plot$`Log Price`),size=1)+
  geom_line(aes(Expect_NJLSPrice_long$Date,log(Expect_NJLSPrice_long$Price)),colour = "#FF8C00",size=0.8) +
  xlab('Date: 2020')+ylab('ln[p(t)]') + theme_bw() +guides(color = FALSE)
p + scale_x_date(date_labels = "%b", date_breaks = "1 month") + geom_vline(xintercept = as.Date("2020-02-15"), color = "red", size=1) + geom_vline(xintercept = as.Date("2020-03-30"), color = "green", size=1) + geom_text(aes(x = as.Date("2020-02-15") , y= 8.6, label = "t1")) + geom_text(aes(x = as.Date("2020-03-30") , y= 8.6, label = "t2")) 
