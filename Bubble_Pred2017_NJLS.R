...
#Initial Estimates of A, B, C1 , C2 & C3 through Least Squares
...
p + geom_histogram(binwidth = 3, fill = "lightblue", colour = "black") + xlim(0,30) +  theme_bw() +
  xlab( "Days to Peak Period") + ylab("Frequency")

for (i in 1:n){
  Expected_BitPrice <- dplyr::bind_rows(Expected_BitPrice,data.frame(t(FittedNJLSPrice(Bitcoin_data,result_test[i,]))))
}
...
Expected_BitPrice_long <- subset(Expected_BitPrice,Expected_BitPrice$Date>= as.Date('2017-07-01'))
...
#Plot the expected bitcoin price
Bitcoin_data_plot <- subset(Bitcoin_data,Bitcoin_data$Date >= as.Date('2017-07-01') & Bitcoin_data$Date <= as.Date('2018-01-01'))
...
p  <- ggplot() + geom_line(aes(Bitcoin_data_plot$Date,Bitcoin_data_plot$`Log Price`),size=1)+
  ...
  xlab('Date:2017- 2018')+ylab('ln[p(t)]')+ guides(color=FALSE) + theme_bw() 
p + scale_x_date(date_labels = "%b", date_breaks = "1 month") + geom_vline(xintercept = as.Date("2017-12-01"), color = "blue", size=1) + geom_vline(xintercept = as.Date("2017-12-17"), color = "red", size=1) +   geom_vline(xintercept = as.Date("2017-09-02"), color = "green", size=1) + geom_text(aes(x = as.Date("2017-09-02") , y= 7.8, label = "t1")) + geom_text(aes(x = as.Date("2017-12-01") , y= 7.8, label = "t2")) +  geom_text(aes(x = as.Date("2017-12-17") , y= 7.8, label = "tc"))
...
Expected_BitPrice_long_CF <- subset(Expected_BitPrice_long, Expected_BitPrice_long$Date >=as.Date('2017-12-09') & Expected_BitPrice_long$Date <=as.Date('2017-12-31'))
